...
<div id="tp-dashboard" class="tooltip-panel">
  <h4>🏠 Dashboard</h4>
  <p>Platform overview with live qualification alerts and today/tomorrow flight roster.</p>
  <div class="use-when">USE WHEN: Starting your shift or checking overnight alerts</div>
</div>
<div id="tp-roster" class="tooltip-panel">
  <h4>📊 Network Roster Grid</h4>
  <p>30-day rolling grid showing all flights vs dates with crew assignments and legality indicators.</p>
  <div class="use-when">USE WHEN: Validating coverage, detecting gaps or crew clustering</div>
</div>
<div id="tp-override" class="tooltip-panel">
  <h4>✏️ Manual Override</h4>
  <p>Replace crew on specific flights with full legality checking and automatic conflict detection.</p>
  <div class="use-when">USE WHEN: Sick call, aircraft swap, or emergency crew change</div>
</div>

<script>
(function() {
  function attachTooltips() {
    const navLinks = window.parent.document.querySelectorAll('[data-testid="stSidebarNavLink"]');
    const tooltipMap = {
      'dashboard': 'tp-dashboard',
      'roster': 'tp-roster',
      'override': 'tp-override',
    };
    navLinks.forEach(link => {
      const text = link.innerText.toLowerCase().replace(/[^a-z]/g, '');
      const tipId = tooltipMap[text];
      if (!tipId) return;
      const tip = document.getElementById(tipId);
      if (!tip) return;
      link.addEventListener('mouseenter', () => {
        document.querySelectorAll('.tooltip-panel').forEach(t => t.style.display = 'none');
