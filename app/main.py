...
st.sidebar.title("✈️ Cabin Crew Roster")
st.sidebar.markdown("---")
...
<style>
  ...
  .metric-card.violation .metric-value { color: #ff4444; }
  .grid-wrapper {
    overflow-x: auto;
    border: 1px solid #1a3a5c;
    border-radius: 6px;
  ...
  .roster-grid tr:hover td { background: #0d2035; }
  .roster-grid tr:hover td.flight-id { background: #0a1a2e; }
  .crew-cell {
    display: flex;
  ...
  .lcc { color: #ffd166; }
  .cc  { color: #06d6a0; }
  .empty-cell { color: #1a3a5c; }
  .override { color: #ff6b6b !important; }
  .live-badge {
...
# Header
st.markdown('<div class="occ-title">✈ AI Crew Roster System</div>', unsafe_allow_html=True)
...
try:
    ...
    <div class="legend">
      <span style="color:#ffd166">■</span> LCC &nbsp;&nbsp;
      <span style="color:#06d6a0">■</span> CC &nbsp;&nbsp;
      <span style="color:#ff6b6b">■</span> Manual Override
    </div>
    """
